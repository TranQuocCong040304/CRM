...
public class ProjectController extends HttpServlet {
	...
	public void projectDetail(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// NhÃ³m tasks theo user.fullName vÃ  task.status.statusName
	    Map<String, Map<String, List<TaskEntity>>> groupedTasks = tasks.stream()
	        .collect(Collectors.groupingBy(
	            task -> task.getUser().getFullName(), // NhÃ³m theo user.fullName
	            Collectors.groupingBy(task -> task.getStatus().getStatusName()) // NhÃ³m con theo statusName
	        ));
